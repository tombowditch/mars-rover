...
I chose to write the mars rover in Go because of the excellent unit testing package and the overall readability of the code. Compile time is also an important factor, and Go compiles extremely quickly (compared to, for example, Rust)
...
- output logging at each stage to see exactly where and how the rover is moving and also an interface to display this
- warning logs if the rover tries to go off the edge of the plateau
- checks to make sure rovers cannot move/start on top of each other
- implement parsing more inline with the code (potentially with struct functions, rather than just calling ParseXYZ in the Start func)
